...
use blit::{Axis, Constraints, Layout, LayoutCx, Platform, Point, Sense, Sides, Size, Sizing, WidgetId};
    ...
    let mut act = None;
    {
        ...
        if line.is_none() {
            for (i, outdated) in file.outdated.iter().enumerate() {
                area.child(flex::item()).build(|ui: Ui<'_>| outdated_box(ui, &file.diff.path, i, outdated, user, &mut act));
            }
        }
        for (i, comment) in file.comments.iter_mut().enumerate().filter(|(_, comment)| comment.at(line)) {
            area.child(flex::item()).build(|ui: Ui<'_>| {
                ...
                card.child(flex::item()).build(|ui: Ui<'_>| card_head(ui, user, &tags[..if comment.earlier { 2 } else { 1 }], place));
                let dismissed = card.child(flex::item()).build(|ui: Ui<'_>| {
                    let mut body = ui.layout(single::layout().padding(Sides::xy(sz::LG, sz::MD)));
                    body.child(single::item().width(Sizing::grow())).build(TextArea {
                ...
                match card.child(flex::item()).build(|ui: Ui<'_>| card_actions(ui, id, &[("Delete", Look::Plain), ("Done", Look::Plain)])) {
...
fn card_head(ui: Ui<'_>, user: &str, tags: &[(&str, Color)], place: &str) {
    ...
    head.child(flex::item()).insert(widgets::text(user, theme::bold(sz::TEXT_SMALL), theme::TEXT));
    for &(label, color) in tags {
        head.child(flex::item()).build(Tag { label, color });
    }
    let options = TextOptions { horizontal_align: HorizontalAlign::Right, ..TextOptions::default() };
    head.child(flex::item().width(Sizing::grow())).insert(widgets::text(place, theme::mono(sz::CODE), theme::MUTED).options(options));
}

fn card_text(ui: Ui<'_>, value: &str) {
    let mut body = ui.layout(single::layout().padding(Sides::xy(sz::LG, sz::MD)));
    body.child(single::item().width(Sizing::grow())).insert(widgets::wrapped(value, theme::interface(sz::TEXT_BODY), theme::TEXT));
}
...
fn outdated_box(ui: Ui<'_>, path: &str, i: usize, outdated: &Outdated, user: &str, act: &mut Option<Act>) {
    ...
    if !outdated.quote.is_empty() {
        ...
        card.child(flex::item()).build(|ui: Ui<'_>| {
            ...
            block.child(single::item().width(Sizing::grow())).insert(widgets::text(&quote, theme::mono(sz::CODE), theme::MUTED));
        });
    }
